...
export const getUsersAndTripsStats = async (): Promise<DashboardStats> => {
  ...
  const endPrevMonth = new Date(d.getFullYear(), d.getMonth(), 0).toISOString();
...
export const getUserItineraryCount = async () => {
  const trips = await database.listDocuments(
    appwriteConfig.databaseId,
    appwriteConfig.tripCollectionId
  );

  const itineraryCount = trips.documents.reduce(
    (acc: { [userId: string]: number }, trip: Document) => {
  ...
  return Object.entries(itineraryCount).map(([userId, count]) => ({
    userId,
    count,
  }));
};
